...
public class IndicatorSeekBar extends View {
    ...
    private static final int DEFAULT_PROGRESS_STYLE = 0;
    private static final int MUSIC_PROGRESS_STYLE = 1;
    private static final int MUSIC_THUMB_TYPE = 0;
    private static final int VIDEO_THUMB_TYPE = 1;
    ...
    private int mIndicatorTopViewType;
    ...
    private Drawable mPressThumbDrawable;
    private Bitmap mNormalThumbBitmap;
    private Bitmap mPressThumbBitmap;
    ...
    private boolean mShouldShowDraggerTime = false;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mProgressStyle = ta.getInt(R.styleable.IndicatorSeekBar_isb_progress_style, DEFAULT_PROGRESS_STYLE);
        ...
        mNormalThumbDrawable = ta.getDrawable(R.styleable.IndicatorSeekBar_isb_normal_thumb_drawable);
        mPressThumbDrawable = ta.getDrawable(R.styleable.IndicatorSeekBar_isb_press_thumb_drawable);
        mThumbType = ta.getInt(R.styleable.IndicatorSeekBar_isb_thumb_type, MUSIC_THUMB_TYPE);
        //thumb text
        ...
        mIndicatorTopViewType = ta.getInt(R.styleable.IndicatorSeekBar_isb_indicator_top_view_type, builder.indicatorTopViewType);
        ...
        mTimeTextSize = ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_indicator_time_text_size, 0);
    ...
    private void initTimePaint() {
        if (mTimePaint == null) {
            ...
            mTimePaint.getTextBounds("0", 0, 1, mTimeRect);
        }
    }
    ...
    public void showMusicPressState(boolean show) {
        mShouldShowDraggerTime = show;
        if (mThumbType == VIDEO_THUMB_TYPE) {
            mIsTouching = show;
        }
        invalidate();
    }

    public void resetDraggerTimeFlag() {
        mShouldShowDraggerTime = true;
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (mProgressStyle == DEFAULT_PROGRESS_STYLE) {
            drawThumb(canvas);
        } else if (mProgressStyle == MUSIC_PROGRESS_STYLE) {
            drawMusicThumb(canvas);
        }
    ...
    private void drawTrack(Canvas canvas) {
        if (mCustomTrackSectionColorResult) {//the track has custom the section track color
            ...
            if (mProgressStyle == MUSIC_PROGRESS_STYLE) {
                if (mNormalThumbBitmap != null) {
                    canvas.drawLine(mProgressTrack.left - mNormalThumbBitmap.getWidth() / 2f,
                            mBackgroundTrack.top, mProgressTrack.left, mBackgroundTrack.bottom, mStockPaint);
                    canvas.drawLine(mBackgroundTrack.right, mBackgroundTrack.top,
                            mBackgroundTrack.right + mNormalThumbBitmap.getWidth() / 2f, mBackgroundTrack.bottom, mStockPaint);
                }
            }
        }
    }
    ...
    private String getProgressTimeText() {
        ...
        return format((long) mProgress) + "-" + format((long) mMax);
    }
    ...
    private void drawMusicThumb(Canvas canvas) {
        if (mHideThumb) {
            return;
        }
        float thumbCenterX = getThumbCenterX();
        if (mNormalThumbDrawable != null && mPressThumbDrawable != null) {
            if (mNormalThumbBitmap == null && mPressedThumbBitmap == null) {
                if (mThumbType == MUSIC_THUMB_TYPE) {
                    mPressThumbDrawable.setAlpha(25);
                }
                mNormalThumbBitmap = drawableToBitmap(mNormalThumbDrawable);
                mPressedThumbBitmap = drawableToBitmap(mPressThumbDrawable);
            }
        }
        mStockPaint.setColor(mThumbColor);
        if (mThumbType == MUSIC_THUMB_TYPE) { //music type
            if (mIsTouching) {
                if (mPressedThumbBitmap != null) {
                    canvas.drawBitmap(mPressedThumbBitmap, thumbCenterX - mPressedThumbBitmap.getWidth() / 2.0f, mProgressTrack.top - mPressedThumbBitmap.getHeight() / 2.0f, mStockPaint);
                    if (mNormalThumbBitmap != null) {
                        canvas.drawBitmap(mNormalThumbBitmap, thumbCenterX - mNormalThumbBitmap.getWidth() / 2.0f, mProgressTrack.top - mNormalThumbBitmap.getHeight() / 2.0f, mStockPaint);
                    }
                }
            } else {
                if (mShouldShowDraggerTime) {
                    updateIndicator();
        ...
        } else if (mThumbType == VIDEO_THUMB_TYPE) { //video type
            if (mIsTouching) {
                if (mPressedThumbBitmap != null) {
                    canvas.drawBitmap(mPressedThumbBitmap, thumbCenterX - mPressedThumbBitmap.getWidth() / 2.0f, mProgressTrack.top - mPressedThumbBitmap.getHeight() / 2.0f, mStockPaint);
                    if (mShouldShowDraggerTime) {
                        float y = mPressedThumbBitmap.getHeight() / 2f - mTimeRect.height() / 2f + mProgressTrack.top - mPressedThumbBitmap.getHeight() / 2f + mTimeRect.height();
                        canvas.drawText(getProgressTimeText(), thumbCenterX, y, mTimePaint);
                    }
                }
            } else {
                if (mNormalThumbBitmap != null) {
